<?php namespace eduTrac\Classes\tcpdf;
//============================================================+
// File name   : Tcpdf.php
// Version     : 6.0.023
// Begin       : 2002-08-03
...
use \eduTrac\Classes\tcpdf\TcpdfBarcode;
use \eduTrac\Classes\tcpdf\Tcpdf2dBarcode;
use \eduTrac\Classes\tcpdf\TcpdfFontData;
use \eduTrac\Classes\tcpdf\TcpdfFonts;
use \eduTrac\Classes\tcpdf\TcpdfColors;
use \eduTrac\Classes\tcpdf\TcpdfImages;
use \eduTrac\Classes\tcpdf\TcpdfStatic;
...
class Tcpdf {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		TcpdfFonts::utf8Bidi(array(''), '', false, $this->isunicode, $this->CurrentFont);
		// set default font
	...
	public function Header() {
		if ($this->header_xobjid < 0) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$imgtype = TcpdfImages::getImageFileType(K_PATH_IMAGES.$headerdata['logo']);
	...
	public function setSpotColor($type, $name, $tint=100) {
		$spotcolor = TcpdfColors::getSpotColor($name, $this->spot_colors);
		if ($spotcolor === false) {
	...
	public function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		return $this->GetArrStringWidth(TcpdfFonts::utf8Bidi(TcpdfFonts::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont), $s, $this->tmprtl, $this->isunicode, $this->CurrentFont), $fontname, $fontstyle, $fontsize, $getarray);
	}
	...
	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		...
		if ($this->isunicode AND (!$this->isUnicodeFont())) {
			$sa = TcpdfFonts::UTF8ArrToLatin1Arr($sa);
		}
		$w = 0; // total width
	...
	public function GetNumChars($s) {
		if ($this->isUnicodeFont()) {
			return count(TcpdfFonts::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont));
		}
		return strlen($s);
	}
	...
	protected function getFontsList() {
		if (($fontsdir = opendir(TcpdfFonts::_getfontpath())) !== false) {
	...
	public function unichr($c) {
		return TcpdfFonts::unichr($c, $this->isunicode);
	}
	...
	public function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false) {
		return TcpdfFonts::addTTFfont($fontfile, $fonttype, $enc, $flags, $outpath, $platid, $encid, $addcbbox);
	}
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (TcpdfStatic::empty_string($fontfile) OR (!@file_exists($fontfile))) {
			// build a standard filenames for specified font
			$tmp_fontfile = str_replace(' ', '', $family).strtolower($style).'.php';
			// search files on various directories
			if (($fontdir !== false) AND @file_exists($fontdir.$tmp_fontfile)) {
				$fontfile = $fontdir.$tmp_fontfile;
			} elseif (@file_exists(TcpdfFonts::_getfontpath().$tmp_fontfile)) {
				$fontfile = TcpdfFonts::_getfontpath().$tmp_fontfile;
			} elseif (@file_exists($tmp_fontfile)) {
				$fontfile = $tmp_fontfile;
			} elseif (!TcpdfStatic::empty_string($style)) {
				$missing_style = true;
	...
	public function isCharDefined($char, $font='', $style='') {
		if (is_string($char)) {
			// get character code
			$char = TcpdfFonts::UTF8StringToArray($char, $this->isunicode, $this->CurrentFont);
			$char = $char[0];
		}
	...
	public function replaceMissingChars($text, $font='', $style='', $subs=array()) {
		...
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
		$uniarr = TcpdfFonts::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		...
		return TcpdfFonts::UniArrSubString(TcpdfFonts::UTF8ArrayToUniArray($uniarr, $this->isunicode));
	}
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!TcpdfStatic::empty_string($opt['FS']))
				AND (@file_exists($opt['FS']) OR TcpdfStatic::isValidURL($opt['FS']))
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$txt = str_replace(TcpdfFonts::unichr(160, $this->isunicode), ' ', $txt);
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					$txt2 = TcpdfFonts::UTF8ToLatin1($txt2, $this->isunicode, $this->CurrentFont);
				} else {
					$unicode = TcpdfFonts::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); // array of UTF-8 unicode values
					$unicode = TcpdfFonts::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
					...
					$txt2 = TcpdfFonts::arrUTF8ToUTF16BE($unicode, false);
				}
			}
			...
			if (isset($uniblock)) {
				...
				foreach ($uniblock as $uk => $uniarr) {
					if (($uk % 2) == 0) {
						...
						// character to print
						$topchr = TcpdfFonts::arrUTF8ToUTF16BE($uniarr, false);
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		...
		$lines = 1;
		$sum = 0;
		$chars = TcpdfFonts::utf8Bidi(TcpdfFonts::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont), $txt, $this->tmprtl, $this->isunicode, $this->CurrentFont);
		$charsWidth = $this->GetArrStringWidth($chars, '', '', 0, true);
		$length = count($chars);
		$lastSeparator = -1;
		for ($i = 0; $i < $length; ++$i) {
			$charWidth = $charsWidth[$i];
			if (preg_match($this->re_spaces, TcpdfFonts::unichr($chars[$i], $this->isunicode))) {
				$lastSeparator = $i;
			}
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		$chars = TcpdfFonts::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		// calculate maximum width for a single character on string
		$chrw = $this->GetArrStringWidth($chars, '', '', 0, true);
		array_walk($chrw, array($this, 'getRawCharWidth'));
		$maxchwidth = max($chrw);
		// get array of chars
		$uchars = TcpdfFonts::UTF8ArrayToUniArray($chars, $this->isunicode);
		// get the number of characters
		$nb = count($chars);
		// replacement for SHY character (minus symbol)
		$shy_replacement = 45;
		$shy_replacement_char = TcpdfFonts::unichr($shy_replacement, $this->isunicode);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				$tmpstr = TcpdfFonts::UniArrSubString($uchars, $j, $i);
				if ($firstline) {
					$startx = $this->x;
					$tmparr = array_slice($chars, $j, ($i - $j));
					if ($rtlmode) {
						$tmparr = TcpdfFonts::utf8Bidi($tmparr, $tmpstr, $this->tmprtl, $this->isunicode, $this->CurrentFont);
					}
				...
				if ($firstline) {
					...
					return (TcpdfFonts::UniArrSubString($uchars, $i));
				}
				++$nl;
				$j = $i + 1;
				$l = 0;
				...
				if (($c != 160)
					AND (($c == 173)
						OR preg_match($this->re_spaces, TcpdfFonts::unichr($c, $this->isunicode))
						OR (($c == 45)
							AND ($i < ($nb - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], TcpdfFonts::unichr($pc, $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], TcpdfFonts::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
				...
				if ($this->isUnicodeFont() AND ($arabic)) {
					...
					// *** very slow ***
					$l = $this->GetArrStringWidth(TcpdfFonts::utf8Bidi(array_slice($chars, $j, ($i - $j)), '', $this->tmprtl, $this->isunicode, $this->CurrentFont));
				} else {
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) > $wmax)) ) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							$linebreak = true;
							if ($firstline) {
								return (TcpdfFonts::UniArrSubString($uchars, $j));
							}
						} else {
						...
						$strrest = TcpdfFonts::UniArrSubString($uchars, ($sep + $endspace));
						...
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							...
							$tmpstr = TcpdfFonts::UniArrSubString($uchars, $j, ($sep + $endspace));
							if ($firstline) {
								$startx = $this->x;
							...
							if ($firstline) {
								...
								$this->cell_padding = $tmpcellpadding;
								return (TcpdfFonts::UniArrSubString($uchars, ($sep + $endspace)));
							}
							$i = $sep;
							$sep = -1;
							$shy = false;
		...
		if ($l > 0) {
			...
			$tmpstr = TcpdfFonts::UniArrSubString($uchars, $j, $nb);
			if ($firstline) {
				$startx = $this->x;
			...
			if ($firstline) {
				$this->cell_padding = $tmpcellpadding;
				return (TcpdfFonts::UniArrSubString($uchars, $nb));
			}
			++$nl;
		}
		if ($firstline) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (isset($imgdata) AND ($imgdata !== FALSE)) {
			// copy image to cache
			$file = TcpdfStatic::getObjFilename('img');
			$fp = fopen($file, 'w');
			fwrite($fp, $imgdata);
			fclose($fp);
			unset($imgdata);
		...
		if ($newimage) {
			...
			if ($type == '') {
				$type = TcpdfImages::getImageFileType($file, $imsize);
			} elseif ($type == 'jpg') {
				$type = 'jpeg';
			}
			$mqr = TcpdfStatic::get_mqr();
			TcpdfStatic::set_mqr(false);
			// Specific image handlers (defined on TcpdfImages CLASS)
			$mtd = '_parse'.$type;
			// GD image handler function
			$gdfunction = 'imagecreatefrom'.$type;
			$info = false;
			if ((method_exists('TcpdfImages', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				// TCPDF image functions
				$info = TcpdfImages::$mtd($file);
				...
				if (function_exists($gdfunction)) {
					...
					if ($resize) {
						...
						if (($type == 'gif') OR ($type == 'png')) {
							$imgr = TcpdfImages::setGDImageTransparency($imgr, $img);
						}
						imagecopyresampled($imgr, $img, 0, 0, 0, 0, $neww, $newh, $pixw, $pixh);
						if (($type == 'gif') OR ($type == 'png')) {
							$info = TcpdfImages::_toPNG($imgr);
						} else {
							$info = TcpdfImages::_toJPEG($imgr, $this->jpeg_quality);
						}
					} else {
						if (($type == 'gif') OR ($type == 'png')) {
							$info = TcpdfImages::_toPNG($img);
						} else {
							$info = TcpdfImages::_toJPEG($img, $this->jpeg_quality);
						}
					}
					...
					$info = TcpdfImages::_parsejpeg($tempname);
					unlink($tempname);
					$img->destroy();
				} else {
					return;
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			$tempdoc = TcpdfStatic::getObjFilename('tmppdf');
			$f = fopen($tempdoc, 'wb');
			if (!$f) {
			...
			$tempsign = TcpdfStatic::getObjFilename('tmpsig');
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		if ($destroyall AND isset($this->diskcache) AND $this->diskcache AND (!$preserve_objcopy) AND (!TcpdfStatic::empty_string($this->buffer))) {
			// remove buffer file from cache
			unlink($this->buffer);
		}
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		if ($this->isunicode) {
			$alias['u'][] = TcpdfStatic::_escape(TcpdfFonts::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = TcpdfStatic::_escape(TcpdfFonts::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = TcpdfStatic::_escape(TcpdfFonts::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = TcpdfStatic::_escape(TcpdfFonts::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
			...
			foreach ($alias as $a) {
				...
				if (($pos = strpos($page, $a)) !== false) {
					...
					if ($type == 'u') {
						...
						$shift = TcpdfFonts::UTF8ToUTF16BE($shift, false, $this->isunicode, $this->CurrentFont);
					} else {
	...
	protected function _putpages() {
		...
		$ptpu = TcpdfFonts::UTF8ToUTF16BE($ptpa, false, $this->isunicode, $this->CurrentFont);
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpu = TcpdfFonts::UTF8ToUTF16BE($pnpa, false, $this->isunicode, $this->CurrentFont);
			...
			if (!empty($this->pagegroups)) {
				if (isset($this->newpagegroup[$n])) {
					...
					$ptgu = TcpdfFonts::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pagegroupnum;
				$pnga = TcpdfStatic::formatPageNumber($pagegroupnum);
				$pngu = TcpdfFonts::UTF8ToUTF16BE($pnga, false, $this->isunicode, $this->CurrentFont);
			...
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (isset($pl['opt']['c']) AND (is_array($pl['opt']['c'])) AND !empty($pl['opt']['c'])) {
						$annots .= ' /C '.TcpdfColors::getColorStringFromArray($pl['opt']['c']);
					}
					//$annots .= ' /StructParent ';
					//$annots .= ' /OC ';
						...
						case 'link': {
							if (is_string($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /Dest /'.TcpdfStatic::encodeNameObject(substr($pl['txt'], 1));
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
						...
						case 'widget': {
							...
							if (isset($pl['opt']['mk']) AND (is_array($pl['opt']['mk'])) AND !empty($pl['opt']['mk'])) {
								...
								if (isset($pl['opt']['mk']['bc']) AND (is_array($pl['opt']['mk']['bc']))) {
									$annots .= ' /BC '.TcpdfColors::getColorStringFromArray($pl['opt']['mk']['bc']);
								}
								if (isset($pl['opt']['mk']['bg']) AND (is_array($pl['opt']['mk']['bg']))) {
									$annots .= ' /BG '.TcpdfColors::getColorStringFromArray($pl['opt']['mk']['bg']);
								}
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			} elseif (@file_exists(TcpdfFonts::_getfontpath().$file)) {
				$fontfile = TcpdfFonts::_getfontpath().$file;
			...
			if (!TcpdfStatic::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					$font = TcpdfFonts::_getTrueTypeFontSubset($font, $subsetchars);
	...
	protected function _puttruetypeunicode($font) {
		...
		$out .= "\n".TcpdfFonts::_putfontwidths($font, 0);
		...
		if (isset($font['ctg']) AND (!TcpdfStatic::empty_string($font['ctg']))) {
			...
			} elseif (@file_exists(TcpdfFonts::_getfontpath().$ctgfile)) {
				$fontfile = TcpdfFonts::_getfontpath().$ctgfile;
	...
	protected function _putcidfont0($font) {
		...
		$out .= ' /FontDescriptor '.($this->n + 1).' 0 R';
		$out .= ' /DW '.$font['dw'];
		$out .= "\n".TcpdfFonts::_putfontwidths($font, $cidoffset);
		$out .= ' >>';
		$out .= "\n".'endobj';
		$this->_out($out);
		$this->_newobj();
	...
	protected function _putXMP() {
		...
		$this->encrypted = false;
		// set XMP data
		$xmp = '<?xpacket begin="'.TcpdfFonts::unichr(0xfeff, $this->isunicode).'" id="W5M0MpCehiHzreSzNTczkc9d"?>'."\n";
		...
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.TcpdfStatic::_escapeXML($this->keywords).' TCPDF</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		...
		$xmp .= "\t\t\t".'<pdf:Keywords>'.TcpdfStatic::_escapeXML($this->keywords).' TCPDF</pdf:Keywords>'."\n";
	...
	protected function _textstring($s, $n=0) {
		if ($this->isunicode) {
			...
			$s = TcpdfFonts::UTF8ToUTF16BE($s, true, $this->isunicode, $this->CurrentFont);
		}
	...
	protected function _escapetext($s) {
		if ($this->isunicode) {
			if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
				$s = TcpdfFonts::UTF8ToLatin1($s, $this->isunicode, $this->CurrentFont);
			} else {
				//Convert string to UTF-16BE and reverse RTL language
				$s = TcpdfFonts::utf8StrRev($s, false, $this->tmprtl, $this->isunicode, $this->CurrentFont);
			}
		}
		return TcpdfStatic::_escape($s);
	}
	...
	protected function _fixAES256Password($password) {
		...
		$psw_array = TcpdfFonts::utf8Bidi(TcpdfFonts::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
		foreach ($psw_array as $c) {
			$psw .= TcpdfFonts::unichr($c, $this->isunicode);
		}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				$tempkeyfile = TcpdfStatic::getObjFilename('tmpkey');
				...
				fclose($f);
				$tempencfile = TcpdfStatic::getObjFilename('tmpenc');
	...
	protected function getCSSBorderStyle($cssborder) {
		...
		$border['color'] = TcpdfColors::convertHTMLColorToDec($color, $this->spot_colors);
		return $border;
	}
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
		$refsize = TcpdfFonts::getFontRefSize($refsize);
		$parent_size = TcpdfFonts::getFontRefSize($parent_size, $refsize);
		switch ($val) {
			case 'xx-small': {
				$size = ($refsize - 4);
				break;
	...
	protected function getHtmlDomArray($html) {
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element{0} == '/') {
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						if (isset($dom[$key]['style']['color']) AND (!TcpdfStatic::empty_string($dom[$key]['style']['color']))) {
							$dom[$key]['fgcolor'] = TcpdfColors::convertHTMLColorToDec($dom[$key]['style']['color'], $this->spot_colors);
						...
						if (isset($dom[$key]['style']['background-color']) AND (!TcpdfStatic::empty_string($dom[$key]['style']['background-color']))) {
							$dom[$key]['bgcolor'] = TcpdfColors::convertHTMLColorToDec($dom[$key]['style']['background-color'], $this->spot_colors);
						}
						// text-decoration
						...
						if (isset($dom[$key]['style']['border-color'])) {
							...
							if (isset($brd_colors[3])) {
								$dom[$key]['border']['L']['color'] = TcpdfColors::convertHTMLColorToDec($brd_colors[3], $this->spot_colors);
							}
							if (isset($brd_colors[1])) {
								$dom[$key]['border']['R']['color'] = TcpdfColors::convertHTMLColorToDec($brd_colors[1], $this->spot_colors);
							}
							if (isset($brd_colors[0])) {
								$dom[$key]['border']['T']['color'] = TcpdfColors::convertHTMLColorToDec($brd_colors[0], $this->spot_colors);
							}
							if (isset($brd_colors[2])) {
								$dom[$key]['border']['B']['color'] = TcpdfColors::convertHTMLColorToDec($brd_colors[2], $this->spot_colors);
							}
						}
						...
						foreach ($cellside as $bsk => $bsv) {
							...
							if (isset($dom[$key]['style']['border-'.$bsv.'-color'])) {
								$dom[$key]['border'][$bsk]['color'] = TcpdfColors::convertHTMLColorToDec($dom[$key]['style']['border-'.$bsv.'-color'], $this->spot_colors);
							}
					...
					if (isset($dom[$key]['attribute']['color']) AND (!TcpdfStatic::empty_string($dom[$key]['attribute']['color']))) {
						$dom[$key]['fgcolor'] = TcpdfColors::convertHTMLColorToDec($dom[$key]['attribute']['color'], $this->spot_colors);
					...
					if (isset($dom[$key]['attribute']['bgcolor']) AND (!TcpdfStatic::empty_string($dom[$key]['attribute']['bgcolor']))) {
						$dom[$key]['bgcolor'] = TcpdfColors::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
					}
					// set stroke color attribute
					if (isset($dom[$key]['attribute']['strokecolor']) AND (!TcpdfStatic::empty_string($dom[$key]['attribute']['strokecolor']))) {
						$dom[$key]['strokecolor'] = TcpdfColors::convertHTMLColorToDec($dom[$key]['attribute']['strokecolor'], $this->spot_colors);
					}
					// check for width attribute
				...
				$dom[$key]['tag'] = false;
				$dom[$key]['block'] = false;
				//$element = str_replace('&nbsp;', TcpdfFonts::unichr(160, $this->isunicode), $element);
	...
	public function serializeTCPDFtagParameters($pararray) {
		return TcpdfStatic::serializeTCPDFtagParameters($pararray);
	}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
			...
			case 'img': {
				if (isset($tag['attribute']['src'])) {
					if ($tag['attribute']['src']{0} === '@') {
						...
						$type = TcpdfImages::getImageFileType($tag['attribute']['src']);
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'lower-greek': {
				$textitem = TcpdfFonts::unichr((945 + $this->listcount[$this->listnum] - 1), $this->isunicode);
				break;
			}
			/*
	...
	protected function setBuffer($data) {
		...
		if ($this->diskcache) {
			if (!isset($this->buffer) OR TcpdfStatic::empty_string($this->buffer)) {
				$this->buffer = TcpdfStatic::getObjFilename('buffer');
			}
	...
	protected function setPageBuffer($page, $data, $append=false) {
		if ($this->diskcache) {
			if (!isset($this->pages[$page])) {
				$this->pages[$page] = TcpdfStatic::getObjFilename('page'.$page);
			}
	...
	protected function setImageBuffer($image, $data) {
		...
		if ($this->diskcache) {
			if (!isset($this->images[$image])) {
				$this->images[$image] = TcpdfStatic::getObjFilename('image'.$image);
			}
	...
	protected function setFontBuffer($font, $data) {
		if ($this->diskcache) {
			if (!isset($this->fonts[$font])) {
				$this->fonts[$font] = TcpdfStatic::getObjFilename('font');
			}
		...
		if (!TcpdfStatic::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$nu = TcpdfFonts::UTF8ToUTF16BE($na, false, $this->isunicode, $this->CurrentFont);
					// replace aliases with numbers
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen($u) - strlen($nu.' '))));
						if ($this->rtl) {
							$nr = $nu.TcpdfFonts::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
						} else {
							$nr = TcpdfFonts::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
						}
	...
	*/
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word_string = TcpdfFonts::UTF8ArrSubString($word, '', '', $this->isunicode);
		...
		if (isset($dictionary[$word_string])) {
			return TcpdfFonts::UTF8StringToArray($dictionary[$word_string], $this->isunicode, $this->CurrentFont);
		}
		...
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			...
			for ($i = $charmin; $i <= $imax; ++$i) {
				$subword = strtolower(TcpdfFonts::UTF8ArrSubString($tmpword, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subword])) {
					$pattern = TcpdfFonts::UTF8StringToArray($patterns[$subword], $this->isunicode, $this->CurrentFont);
					...
					for ($j = 0; $j < $pattern_length; ++$j) {
						...
						if (($pattern[$j] >= 48) AND ($pattern[$j] <= 57)) {
							...
							if (!isset($hyphenword[$zero]) OR ($hyphenword[$zero] != $pattern[$j])) {
								$hyphenword[$zero] = TcpdfFonts::unichr($pattern[$j], $this->isunicode);
							}
							++$digits;
						}
					}
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$unichars = TcpdfFonts::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		// for each char
		foreach ($unichars as $char) {
			if ((!$intag) AND TcpdfFontData::$uni_type[$char] == 'L') {
				// letter character
				$word[] = $char;
			} else {
		...
		return TcpdfFonts::UTF8ArrSubString($txtarr, '', '', $this->isunicode);
	}
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		// color
		$fill_color = TcpdfColors::convertHTMLColorToDec($svgstyle['color'], $this->spot_colors);
		$this->SetFillColorArray($fill_color);
		// text color
		$text_color = TcpdfColors::convertHTMLColorToDec($svgstyle['text-color'], $this->spot_colors);
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			$fill_color = TcpdfColors::convertHTMLColorToDec($svgstyle['fill'], $this->spot_colors);
		...
		if ($svgstyle['stroke'] != 'none') {
			...
			$stroke_style = array(
				'color' => TcpdfColors::convertHTMLColorToDec($svgstyle['stroke'], $this->spot_colors),
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if (isset($attribs['style']) AND !TcpdfStatic::empty_string($attribs['style'])) {
			// fix style for regular expression
			$attribs['style'] = ';'.$attribs['style'];
		}
		foreach ($prev_svgstyle as $key => $val) {
			if (in_array($key, TcpdfImages::$svginheritprop)) {
		...
		switch($name) {
			...
			case 'stop': {
				...
				$stop_color = isset($svgstyle['stop-color'])?TcpdfColors::convertHTMLColorToDec($svgstyle['stop-color'], $this->spot_colors):'black';
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					// get image type
					$imgtype = TcpdfImages::getImageFileType($img);
					if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
