...
class Hooks {
	...
	*/
	...
	public static function update_option( $option_name, $newvalue ) {
		...
		$q = DB::inst()->query( "UPDATE `et_option` SET `option_value` = '$_newvalue' WHERE `option_name` = '$option_name'" );
	...
	public static function add_option( $name, $value = '' ) {
		...
		DB::inst()->query( "INSERT INTO `et_option` (`option_name`, `option_value`) VALUES ('$name', '$_value')" );
	...
	public static function delete_option( $name ) {
		$results = DB::inst()->query( "SELECT option_id FROM `et_option` WHERE `option_name` = '$name'" );
		if ( is_null($results) || !$results->fetch(\PDO::FETCH_BOTH) )
			return false;
		
		self::do_action( 'delete_option', $option_name );
		
		DB::inst()->query( "DELETE FROM `et_option` WHERE `option_name` = '$name'" );
		return true;
	}
