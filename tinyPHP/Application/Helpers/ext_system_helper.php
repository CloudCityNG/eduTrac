...
use \tinyPHP\Classes\Libraries\Cookies;
	...
	function semester_dropdown($semCode = NULL) {
		...
		foreach( $q as $k => $v ) {
	      	echo '<option value="'.$v['semCode'].'"'.selected( $semCode, _h($v['semCode']), false ).'>'._h($v['semName']).'</option>' . "\n";
		}
	}
	...
	function subj_key_dropdown($subjCode = NULL) {
		...
		foreach( $q as $k => $v ) {
	      	echo '<option value="'.$v['subjCode'].'"'.selected( $subjCode, _h($v['subjCode']), false ).'>'._h($v['subjCode']).' '._h($v['subjName']).'</option>' . "\n";
		}
	}
    ...
    function course_sec_status_select($status = NULL) {
        $select = '<select style="width:60%;" name="status" id="select2_9" required>
                ...
                <option value="' . _t('W') . '"'.selected( $status, _t('W'), false ).'>' . _t('W Withdrew') . '</option>
    ...
    function logError($type,$string,$file,$line) {
        $log = new \tinyPHP\Classes\Libraries\Log;
	...
	function hasPermission($perm) {
		$auth = new \tinyPHP\Classes\Libraries\Cookies;
		//$acl = new \tinyPHP\Classes\Libraries\ACL($auth->getPersonField('personID'));
		$acl = new \tinyPHP\Classes\Libraries\ACL($_SESSION['id']);
    ...
    function head_version_meta() {
        echo "<meta name='generator' content='eduTrac " . CURRENT_EG_VERSION . "'>\n";
    }
    
    function foot_version() {
        echo "v". CURRENT_EG_VERSION;
    }
